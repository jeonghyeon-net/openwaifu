export const schedulerToolDescription =
  "Schedule, list, and cancel one-time or cron-based recurring tasks for current Discord user/session. Each due task runs the saved prompt in a completely fresh clean pi session, then posts the result back to Discord. All times use Korea time (Asia/Seoul).";
export const schedulerToolPromptSnippet =
  "`scheduler`: create, list, or cancel one-time and cron-based scheduled tasks that run later in a fresh clean pi session for current Discord user/session";
