...
export const runtime = { prompt: vi.fn(), runScheduledPrompt: vi.fn(async () => "scheduled reply") };
...
export const createSchedulerService = vi.fn((_args: { client: unknown; tasksFile: string; runTask: (task: Record<string, unknown>) => Promise<string> }) => ({ start: startSchedulerService }));
...
export const createChatGptQuotaStatusService = vi.fn((_args: { onStatusText: (text: string) => void; onError?: (error: Error) => void }) => ({ start: startChatGptQuotaStatusService }));
