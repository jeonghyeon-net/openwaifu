...
describe("discord admin channel", () => {
  it("sends, creates, updates, and deletes channels", async () => {
    ...
    await expect(updateDiscordChannel(client, context, { channelId: "c", name: "renamed", categoryId: "cat", topic: "topic" })).resolves.toContain("Updated channel general (c)");
    await expect(updateDiscordChannel(client, context, { channelId: "c", categoryId: "", topic: "" })).resolves.toContain("Updated channel general (c)");
    await expect(deleteDiscordChannel(client, context, { channelId: "c" })).resolves.toContain("Deleted channel general (c)");
  ...
  it("covers forum, announcement, and voice channel branches", async () => {
    ...
    await expect(updateDiscordChannel(client, context, { channelId: "f", topic: "forum-topic" })).resolves.toContain("Updated channel forum (f)");
    await expect(updateDiscordChannel(client, context, { channelId: "a", topic: "news-topic" })).resolves.toContain("Updated channel news (a)");
  });
  ...
  it("updates channels without optional setters", async () => {
    ...
    await expect(updateDiscordChannel(client, context, { channelId: "b", name: "renamed" })).resolves.toContain("Updated channel bare (b)");
  });
  ...
  it("rejects unsupported topic updates", async () => {
    ...
    await expect(updateDiscordChannel(client, context, { channelId: "v", topic: "x" })).rejects.toThrow("does not support topic");
  });
});
