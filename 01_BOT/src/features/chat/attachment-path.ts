...
export const attachmentDirectoryForScope = (
  repoRoot: string,
  scopeId: string,
) => join(repoRoot, "01_BOT", ".data", "discord-attachments", sanitize(scopeId));

export const attachmentDirectoryForMessage = (
  repoRoot: string,
  scopeId: string,
  messageId: string,
) => join(attachmentDirectoryForScope(repoRoot, scopeId), sanitize(messageId));
